...
func (a *APIBackend) FeeHistory(
	...
	speedLimit, err := core.GetMtOSSpeedLimitPerSecond(state)
	if err != nil {
		return common.Big0, nil, nil, nil, err
	}
...
func (a *APIBackend) SubscribePendingLogsEvent(ch chan<- []*types.Log) event.Subscription {
	//Mantle doesn't really need pending logs. Logs are published as soon as we know them..
